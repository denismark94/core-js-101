...
function timeSpanToString(startDate, endDate) {
  ...
  diff -= seconds * 1000;
  const ms = diff;
  return `${(hours < 10) ? '0' : ''}${hours}:${(minutes < 10) ? '0' : ''}${minutes}:${(seconds < 10) ? '0' : ''}${seconds}.${(ms < 100) ? '0' : ''}${(ms < 10) ? '0' : ''}${ms}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let hourAngle = (date.getUTCHours() % 12) * (360 / 12);
  hourAngle += ((30 * date.getMinutes()) / 60);
